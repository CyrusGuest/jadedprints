  ...
  return (
    <div className='flex md:h-[250px] md:flex-row flex-col gap-10 pl-8 pt-8 text-white absolute bottom w-full bg-black h-[550px] mt-24'>
      ...
      <div>
        ...
        { subscribed ? <h3 className='text-xl font-light'>you're subscribed!</h3> : 
          <div>
            <div className='flex border-2 md:w-auto w-3/4 rounded-lg pl-4 py-3 mt-2'>
            <img src={EmailIcon} alt="" />
            <input value={email} onChange={(e) => setEmail(e.target.value)} className='bg-transparent ml-4 text-xl outline-none' placeholder='email address' type="email" name="email" id="email" />
            </div>
            <button onClick={(e) => handleSubscribe(e)} className='bg-white text-black font-bold text-xl md:w-full w-3/4 p-3 rounded-lg mt-4'>subscribe</button>
          </div>
        }
