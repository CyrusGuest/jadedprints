    ...
    <div>
      <div className='mx-10 md:flex md:mx-20 lg:mx-44 md:gap-10 md:mt-10'>
        ...
        <div className='md:max-w-md cursor-pointer'>
          ...
          <h3 className='text-lg font-light mt-1'>${sizePrices[size]} USD tax not included</h3>

          <div className='mt-4'>
            <h3 className='text-2xl font-light'>size</h3>
            <div className='grid grid-cols-2 gap-2'>
              <button className={`${ (size === '20x30') ? 'bg-black text-white' : '' } border py-2 rounded-lg border-black`} onClick={() => setSize('20x30')}>20" x 30"</button>
              <button className={`${ (size === '16x20') ? 'bg-black text-white' : '' } border py-2 rounded-lg border-black`} onClick={() => setSize('16x20')}>16" x 20"</button>
              <button className={`${ (size === '12x18') ? 'bg-black text-white' : '' } border py-2 rounded-lg border-black`} onClick={() => setSize('12x18')}>12" x 18"</button>
              <button className={`${ (size === '11x14') ? 'bg-black text-white' : '' } border py-2 rounded-lg border-black`} onClick={() => setSize('11x14')}>11" x 14"</button>
            </div>
          </div>
          ...
          <p className='text-lg font-light mt-8'>poster with design including the beauty behind the madness album cover, tracklist, spotify link, and other information</p>
        
          <div className='mt-8'>
            <hr className='border-neutral-400' />
            <div onClick={() => setMaterials(!materials)} className='my-3 flex flex-col'>
              <div className='flex'>
              ...
              { materials ? <p className='mt-2 font-light'>Our posters are made with high-quality photo canvas paper.</p> : '' }
            </div>

            <hr className='border-neutral-400' />
            <div onClick={() => setShipping(!shipping)} className='my-3 flex flex-col'>
              <div className='flex'>
              ...
              { shipping ? <p className='mt-2 font-light'>We offer free shipping in the domestic United States. All sales are final.</p> : '' }
            </div>

            <hr className='border-neutral-400' />
            <div onClick={() => setDimensions(!dimensions)} className='my-3 flex flex-col'>
              <div className='flex'>
              ...
              { dimensions ? <p className='mt-2 font-light'>Our posters come in four different sizes: 11"x14", 12"x18", 16"x20", and 20"x30". All units are inches.</p> : '' }
            </div>
            ...
            <div onClick={() => setCare(!care)} className='my-3 flex flex-col'>
              ...
              { care ? <p className='mt-2 font-light'>Keep in a 62-76 fahrenheit, dehumidifed area for best quality.</p> : ''}
            </div>
      ...
      <div className='md:max-w-lg mx-auto max-w-sm'>
        ...
        <p className='md:text-xl text-center mt-4'>unlike our competition, we offer free shipping on all mainland united states orders; this means the sticker price you see is much closer to what you actually pay.</p>

        <h1 className='md:max-w-4xl md:mx-auto md:text-5xl text-4xl mt-10 font-bold text-center mx-4'>custom designs</h1>
        <p className='md:text-xl text-center mt-4'>we offer a plethora of artists' music featured in our posters designs. if you aren't able to find the poster you want, make a request and we'll make one just for you.</p>
      </div>
